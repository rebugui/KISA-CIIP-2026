...
function Invoke-TiberoWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-03' { return Invoke-TiberoSqlCheck $state "SELECT resource_name||'='||limit FROM dba_profiles WHERE profile='DEFAULT' AND resource_name IN ('PASSWORD_LIFE_TIME','PASSWORD_VERIFY_FUNCTION','FAILED_LOGIN_ATTEMPTS','PASSWORD_LOCK_TIME');" { param($r) if ($r.Output -match '(?im)PASSWORD_VERIFY_FUNCTION=NULL|PASSWORD_LIFE_TIME=UNLIMITED|FAILED_LOGIN_ATTEMPTS=UNLIMITED') { New-TiberoResult 'VULNERABLE' 'Tibero DEFAULT profile password lifetime/complexity/lockout controls are weak.' $r.Output $r.Command } else { New-TiberoResult 'GOOD' 'Tibero DEFAULT profile password controls appear configured.' $r.Output $r.Command } } 'password lifetime and complexity' }
        ...
        'D-07' { $accounts = @($state.Services | ForEach-Object { $_.StartName } | Where-Object { $_ }); if ($accounts -match '^(?i)(LocalSystem|NT AUTHORITY\\SYSTEM)$|Administrator') { return New-TiberoResult 'VULNERABLE' 'Tibero Windows service appears to run with administrator-equivalent OS privileges.' ($accounts -join ', ') 'Inspect Tibero Windows service StartName' }; if ($accounts.Count -gt 0) { return New-TiberoResult 'GOOD' 'Tibero Windows service account is not administrator-equivalent.' ($accounts -join ', ') 'Inspect Tibero Windows service StartName' }; return New-TiberoResult 'MANUAL' 'Tibero service account could not be determined.' $evidence 'Inspect Tibero Windows service StartName' }
        ...
        'D-09' { return Invoke-TiberoSqlCheck $state "SELECT resource_name||'='||limit FROM dba_profiles WHERE resource_name IN ('FAILED_LOGIN_ATTEMPTS','PASSWORD_LOCK_TIME') ORDER BY profile,resource_name;" { param($r) if ($r.Output -match '(?im)FAILED_LOGIN_ATTEMPTS=UNLIMITED|PASSWORD_LOCK_TIME=UNLIMITED') { New-TiberoResult 'VULNERABLE' 'Tibero failed-login lockout profile controls are weak.' $r.Output $r.Command } else { New-TiberoResult 'GOOD' 'Tibero failed-login lockout profile evidence was collected.' $r.Output $r.Command } } 'failed login lockout policy' }
        'D-10' { $lines = @(Get-TiberoConfigLines $state '(?i)LSNR_INVITED_IP|LSNR_DENIED_IP|LISTENER_PORT|LSNR_PORT'); if ($lines -match '(?i)LSNR_INVITED_IP|LSNR_DENIED_IP') { return New-TiberoResult 'GOOD' 'Tibero listener IP restriction evidence was found.' ($lines -join "`n") 'Inspect Tibero listener access controls' }; return New-TiberoResult 'MANUAL' 'Tibero listener IP restriction evidence was not conclusive; confirm firewall/listener policy.' (($lines + $evidence) -join "`n") 'Inspect Tibero listener access controls' }
        ...
        'D-14' { $targets = @($state.TiberoHomes + $state.ConfigFiles | Select-Object -Unique); $acl = foreach ($path in $targets) { Get-TiberoBroadAclEvidence -Path $path -Role 'TiberoPath' }; if ($acl) { return New-TiberoResult 'VULNERABLE' 'Broad local write or unknown ACL evidence was found on Tibero paths.' (($acl | ForEach-Object { "$($_.Path) => $($_.Principal) $($_.Access) ($($_.Rights))" }) -join "`n") 'Get-Acl Tibero directories/files' }; if ($targets.Count -eq 0) { return New-TiberoResult 'MANUAL' 'Tibero paths were not found for ACL assessment.' $evidence 'Get-Acl Tibero directories/files' }; return New-TiberoResult 'GOOD' 'No broad local write ACL entries were found on assessed Tibero paths.' ($targets -join ', ') 'Get-Acl Tibero directories/files' }
        ...
        'D-18' { return Invoke-TiberoSqlCheck $state "SELECT grantee||' '||owner||'.'||table_name||' '||privilege FROM dba_tab_privs WHERE grantee='PUBLIC' AND owner NOT IN ('SYS','SYSCAT') AND ROWNUM <= 100;" { param($r) if ($r.Output -match '(?m)\S') { New-TiberoResult 'VULNERABLE' 'Tibero object permissions granted to PUBLIC were found.' $r.Output $r.Command } else { New-TiberoResult 'GOOD' 'No non-system Tibero object permissions granted to PUBLIC were returned.' 'No rows returned.' $r.Command } } 'public role restriction' }
