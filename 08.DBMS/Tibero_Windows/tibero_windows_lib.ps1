...
function Invoke-TiberoWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-18' { return Invoke-TiberoSqlCheck $state "SELECT granted_role FROM dba_role_privs WHERE grantee='PUBLIC';" { param($r) if ($r.Output -match '(?m)\S') { New-TiberoResult 'VULNERABLE' 'Tibero DBA-level role grants to PUBLIC were found.' $r.Output $r.Command } else { New-TiberoResult 'GOOD' 'No Tibero DBA-level role grants to PUBLIC were returned.' 'No rows returned.' $r.Command } } 'public role restriction' }
