...
function Invoke-OracleWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-11' { return Invoke-OracleSqlCheck $state "SELECT grantee||' '||owner||'.'||table_name||' '||privilege FROM dba_tab_privs WHERE owner IN ('SYS','SYSTEM') AND grantee NOT IN (SELECT grantee FROM dba_role_privs WHERE granted_role='DBA') AND grantee NOT IN ('SYS','SYSTEM') AND ROWNUM <= 100;" { param($r) if ($r.Output -match '(?m)\S') { New-OracleResult 'VULNERABLE' 'Non-DBA Oracle system object privileges were found.' $r.Output $r.Command } else { New-OracleResult 'GOOD' 'No non-DBA Oracle system object privilege evidence was returned.' 'No rows returned.' $r.Command } } 'system table access restriction' }
