...
function Invoke-OracleWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-06' { return Invoke-OracleSqlCheck $state "SELECT 'SHARED:'||username FROM dba_users WHERE username IN ('DBA','ADMIN','OPER','SHARED') OR username LIKE 'APP_%' UNION ALL SELECT 'NONSYS:'||username FROM dba_users WHERE username NOT IN ('SYS','SYSTEM','DBSNMP','SYSMAN','OUTLN','DIP','ORACLE_OCM','APPQOSSYS','WMSYS','EXFSYS','CTXSYS','XDB','ANONYMOUS','SI_INFORMTN_SCHEMA','ORDDATA','ORDPLUGINS','ORDSYS','MDSYS','OLAPSYS','OWBSYS','FLOWS_FILES','APEX_PUBLIC_USER','APEX_040000','MDDATA','OJVMSYS','LBACSYS','AUDSYS','DVF','DVSYS','DBSFWUSER','REMOTE_SCHEDULER_AGENT','SYSBACKUP','GSMADMIN_INTERNAL','SYSKM','SYSDG','GSMCATUSER','SYSRAC') ORDER BY 1;" { param($r) if ($r.Output -match '(?im)^SHARED:\S') { New-OracleResult 'MANUAL' 'Shared or privileged-looking Oracle account names were found (DBA/ADMIN/OPER/SHARED/APP_*); confirm individual-vs-shared account assignment policy manually.' $r.Output $r.Command } elseif ($r.Output -match '(?im)^NONSYS:\S') { New-OracleResult 'MANUAL' 'Non-system Oracle accounts exist, but individual-vs-shared attribution cannot be proven from dba_users metadata alone; confirm per-user account policy/binding manually.' $r.Output $r.Command } else { New-OracleResult 'MANUAL' 'Only Oracle system accounts were found; confirm application/operations users use individual (not shared) accounts manually.' 'No non-system rows returned.' $r.Command } } 'individual DB account assignment' }
        ...
        'D-20' { return Invoke-OracleSqlCheck $state "SELECT owner||'.'||object_name||' ('||object_type||')' FROM dba_objects WHERE owner NOT IN ('SYS','SYSTEM','OUTLN','DBSNMP','XDB','WMSYS','CTXSYS','MDSYS','ORDSYS','ORDDATA','APPQOSSYS','AUDSYS','DBSFWUSER','DIP','DVF','DVSYS','EXFSYS','FLOWS_FILES','GSMADMIN_INTERNAL','GSMCATUSER','LBACSYS','MDDATA','OJVMSYS','OLAPSYS','ORACLE_OCM','ORDPLUGINS','OWBSYS','REMOTE_SCHEDULER_AGENT','SI_INFORMTN_SCHEMA','SPATIAL_CSW_ADMIN_USR','SPATIAL_WFS_ADMIN_USR','SYSBACKUP','SYSDG','SYSKM','SYSRAC','APEX_PUBLIC_USER','ANONYMOUS','PUBLIC') AND owner NOT IN (SELECT grantee FROM dba_role_privs WHERE granted_role='DBA') AND owner NOT LIKE 'APEX\_%' ESCAPE '\\' AND ROWNUM <= 100;" { param($r) if ($r.Output -match '(?m)\S') { New-OracleResult 'MANUAL' 'Non-default Oracle Object Owners (any object type: TABLE/VIEW/PROCEDURE/PACKAGE/FUNCTION/SEQUENCE/SYNONYM/TYPE/TRIGGER/INDEX/...) were found outside the KISA system-account and DBA-role allowlist. KISA criteria_bad: ObjectOwner가 일반 사용자에게도 존재하는 경우 -- confirm each owner is an authorized administrator/application account; otherwise treat as VULNERABLE.' $r.Output $r.Command } else { New-OracleResult 'GOOD' 'No non-default Oracle Object Owner evidence (any object type) was returned outside the KISA system-account and DBA-role allowlist.' 'No rows returned.' $r.Command } } 'unauthorized object owner restriction' }
