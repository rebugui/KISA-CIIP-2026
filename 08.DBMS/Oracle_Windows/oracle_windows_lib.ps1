...
function Invoke-OracleWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-09' { return Invoke-OracleSqlCheck $state "SELECT resource_name||'='||limit FROM dba_profiles WHERE resource_name IN ('FAILED_LOGIN_ATTEMPTS','PASSWORD_LOCK_TIME') ORDER BY profile,resource_name;" { param($r) if ($r.Output -match '(?im)FAILED_LOGIN_ATTEMPTS=(UNLIMITED|0)|PASSWORD_LOCK_TIME=(UNLIMITED|0)') { New-OracleResult 'VULNERABLE' 'Oracle failed-login lockout profile controls are weak.' $r.Output $r.Command } else { New-OracleResult 'GOOD' 'Oracle failed-login lockout profile evidence was collected.' $r.Output $r.Command } } 'failed login lockout policy' }
