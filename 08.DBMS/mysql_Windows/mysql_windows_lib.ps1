...
function Invoke-MySqlWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-02' { return Invoke-MySqlSqlCheck $state "SELECT user,host,account_locked FROM mysql.user WHERE user IN ('test','anonymous','') OR user LIKE 'test%';" { param($r) $rows = @(($r.Output -split "`r?`n") | Where-Object { $_ -match '\S' }); if ($rows.Count -eq 0) { return New-MySqlResult 'GOOD' 'No obvious unnecessary MySQL accounts were found.' 'No rows returned.' $r.Command }; $unlocked = @($rows | Where-Object { $parts = $_ -split "`t"; ($parts.Count -lt 3) -or ($parts[2].Trim() -ne 'Y') }); if ($unlocked.Count -gt 0) { New-MySqlResult 'VULNERABLE' 'Unnecessary MySQL accounts that are NOT locked were found (account_locked != Y).' (($unlocked -join "`n")) $r.Command } else { New-MySqlResult 'GOOD' 'Unnecessary MySQL account names exist but every row is locked (account_locked=Y); per D-02 the lock-and-keep alternative is compliant.' (($rows -join "`n")) $r.Command } } 'unnecessary account removal' }
        'D-03' { return Invoke-MySqlSqlCheck $state "SHOW VARIABLES WHERE Variable_name IN ('default_password_lifetime','validate_password.policy','validate_password.length');" { param($r) $o=$r.Output; if($o -notmatch 'validate_password'){return New-MySqlResult 'VULNERABLE' 'validate_password plugin/component is not installed.' $o $r.Command}; $m=[regex]::Match($o,'validate_password\.length\s+(\S+)'); $len=if($m.Success){[int]$m.Groups[1].Value}else{-1}; $m2=[regex]::Match($o,'validate_password\.policy\s+(\S+)'); $pol=if($m2.Success){$m2.Groups[1].Value}else{''}; $m3=[regex]::Match($o,'default_password_lifetime\s+(\S+)'); $life=if($m3.Success){$m3.Groups[1].Value}else{$null}; $vulns=@(); if($len -ge 0 -and $len -lt 8){$vulns+="length=$len(<8)"}; if($pol -eq '0' -or $pol -eq 'LOW'){$vulns+="policy=$pol"}; if($vulns.Count){return New-MySqlResult 'VULNERABLE' "Password complexity weak: $($vulns-join'; ')." $o $r.Command}; if($null -eq $life -or $life -eq '0' -or $life -eq 'NULL'){return New-MySqlResult 'MANUAL' 'Password lifetime not set; confirm institutional policy.' $o $r.Command}; New-MySqlResult 'GOOD' 'Password lifetime and complexity settings are adequate.' $o $r.Command } 'password lifetime and complexity' }
        ...
        'D-11' { return Invoke-MySqlSqlCheck $state "SELECT Grantee,Table_schema,Privilege_type FROM information_schema.schema_privileges WHERE Table_schema IN ('mysql','performance_schema','sys','information_schema') UNION ALL SELECT GRANTEE, '*' as Table_schema, Privilege_type FROM information_schema.user_privileges WHERE Privilege_type IN ('SELECT','INSERT','UPDATE','DELETE','DROP','ALTER','CREATE','ALL PRIVILEGES');" { param($r) $nonRoot = @(($r.Output -split "`r?`n") | Where-Object { $_ -match '(?m)\S' -and $_ -notmatch "(?i)'root'@" }); if ($nonRoot.Count -gt 0 -and (($nonRoot -join "`n") -match "(?i)PUBLIC|'%'|@'%'")) { New-MySqlResult 'VULNERABLE' 'Broad system schema or global *.* privilege evidence was found (PUBLIC or @%).' (($nonRoot -join "`n")) $r.Command } elseif ($nonRoot.Count -gt 0) { New-MySqlResult 'MANUAL' 'Non-root accounts hold system schema or global privileges; confirm only DBAs are authorized.' (($nonRoot -join "`n")) $r.Command } elseif ($r.Output -match '(?m)\S') { New-MySqlResult 'GOOD' 'Only root-account system/global privileges were returned.' $r.Output $r.Command } else { New-MySqlResult 'GOOD' 'No explicit non-DBA system schema privilege evidence was returned.' 'No rows returned.' $r.Command } } 'system table access restriction' }
        ...
        'D-21' { return Invoke-MySqlSqlCheck $state "SELECT user,host FROM mysql.user WHERE Grant_priv='Y' AND user NOT IN ('root','mysql.sys','mysql.session');" { param($r) if ($r.Output -match '(?m)\S') { New-MySqlResult 'VULNERABLE' 'Non-default MySQL accounts with GRANT OPTION were found; GRANT OPTION should only be granted via ROLE to DBA accounts.' $r.Output $r.Command } else { New-MySqlResult 'GOOD' 'No non-default MySQL accounts with GRANT OPTION were returned.' 'No rows returned.' $r.Command } } 'GRANT OPTION restriction' }
