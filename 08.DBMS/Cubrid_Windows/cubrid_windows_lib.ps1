...
function Invoke-CubridWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-18' { return Invoke-CubridSqlCheck $state "SELECT name FROM db_user WHERE name='PUBLIC';" { param($r) if ($r.Output -match '(?im)^PUBLIC') { New-CubridResult 'MANUAL' 'CUBRID PUBLIC role/user evidence exists; confirm application/DBA roles are not assigned broadly.' $r.Output $r.Command } else { New-CubridResult 'GOOD' 'No CUBRID PUBLIC role/user evidence was returned.' 'No rows returned.' $r.Command } } 'public role restriction' }
