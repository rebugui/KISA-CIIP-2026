...
function Invoke-PostgreSqlWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-13' { $drivers = @(@('HKLM:\SOFTWARE\ODBC\ODBCINST.INI','HKLM:\SOFTWARE\WOW6432Node\ODBC\ODBCINST.INI') | ForEach-Object { Get-ChildItem $_ -ErrorAction SilentlyContinue } | Where-Object { $_.PSChildName -notmatch '(?i)^ODBC Drivers$|^ODBC Translators$' } | Select-Object -ExpandProperty PSChildName -Unique); if ($drivers.Count -gt 0) { return New-PostgreSqlResult 'MANUAL' '설치된 ODBC 드라이버/데이터 소스 항목이 발견되었습니다. 운영상 필요한 항목인지(불필요한 ODBC/OLE-DB가 설치되어 있지 않은지) 수동으로 확인하세요. (criteria_bad: 불필요한 ODBC/OLE-DB가 설치된 경우)' ($drivers -join "`n") 'Inspect Windows ODBC registry driver entries (native + WOW6432Node)' }; return New-PostgreSqlResult 'GOOD' 'No ODBC driver entries were found in the inspected registry paths.' 'No ODBC entries returned.' 'Inspect Windows ODBC registry driver entries (native + WOW6432Node)' }
