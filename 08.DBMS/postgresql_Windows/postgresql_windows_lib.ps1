...
function Invoke-PostgreSqlWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'D-03' { return Invoke-PostgreSqlSqlCheck $state "SELECT rolname, rolvaliduntil FROM pg_authid WHERE rolcanlogin;" { param($r) if ($r.Output -match '(?m)\|$|infinity') { New-PostgreSqlResult 'MANUAL' 'Some PostgreSQL login roles have no finite password validity; confirm institutional password lifetime policy.' $r.Output $r.Command } else { New-PostgreSqlResult 'GOOD' 'PostgreSQL login role validity evidence was collected.' $r.Output $r.Command } } 'password lifetime policy' }
