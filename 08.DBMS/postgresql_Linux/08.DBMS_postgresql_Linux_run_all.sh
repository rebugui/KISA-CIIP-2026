...
# @Platform    : PostgreSQL
# @Severity    : 以?# @Title       : 紐⑤뱺 DBMS ?먭? ?ㅽ겕由쏀듃 ?ㅽ뻾
# @Description : PostgreSQL DBMS 紐⑤뱺 ?먭? ??ぉ???ㅽ뻾?섎뒗 ?ㅽ겕由쏀듃 (Unix ?뺤떇)
...
run_single_check() {
    ...
    # PC ?뺤떇?쇰줈 CLI 異쒕젰 (媛꾨떒???붿빟留?
    echo "==================================================================="
    echo "吏꾨떒 ??ぉ: ${item_id} (${current}/${TOTAL_ITEMS})"
    echo "==================================================================="
    echo "吏꾨떒 ??ぉ: ${item_id} - ${item_name}"
    echo "${summary}"
    echo "  > 吏꾨떒 ?꾨즺: ${final_result}"
    echo ""
