...
diagnose() {
    ...
    if ! systemctl is-active oracle &>/dev/null && ! pgrep -f "ora_pmon" &>/dev/null; then
        ...
        status="수동진단"
        inspection_summary="서비스 미실행으로 AuditTable(AUD\$) 접근권한 확인 불가 (수동진단 필요). 서비스 시작 후 SELECT owner FROM dba_tables WHERE table_name='AUD\$'; 결과가 SYS/SYSTEM 소유인지 수동으로 확인하세요."
