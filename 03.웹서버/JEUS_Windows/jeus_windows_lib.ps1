...
function Invoke-JeusWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'WEB-08' { $lines = @(Get-JeusConfigLines $state '(?i)max-file-size|max-request-size|maxPostSize|multipart-config'); $effective = @($lines | Where-Object { $_ -notmatch '<!--' -and $_ -notmatch '-->' }); if ($effective -match '(?i)(max-file-size|max-request-size|maxPostSize)[^0-9]*[1-9][0-9]*') { return New-JeusResult 'GOOD' 'JEUS upload/request size limit evidence (non-zero numeric value) was found.' ($effective -join "`n") 'Inspect web.xml multipart upload limits' }; if ($lines.Count -gt 0) { return New-JeusResult 'VULNERABLE' 'JEUS multipart/upload configuration was present but had no real non-zero size limit (commented out, empty, or zero value).' ($lines -join "`n") 'Inspect web.xml multipart upload limits' }; return New-JeusResult 'VULNERABLE' 'JEUS upload/request size limit evidence was not found.' $evidence 'Inspect web.xml multipart upload limits' }
        ...
        'WEB-22' { $lines = @(Get-JeusConfigLines $state '(?i)<error-page>|<error-code>|<location>'); $effective = @($lines | Where-Object { $_ -notmatch '<!--' -and $_ -notmatch '-->' }); if ($effective -match '(?i)<location>\s*[^<\s][^<]*</location>') { return New-JeusResult 'GOOD' 'JEUS custom error-page with a designated location value was found.' ($effective -join "`n") 'Inspect JEUS error-page settings' }; if ($effective -match '(?i)<error-page>') { return New-JeusResult 'MANUAL' 'JEUS error-page element was found but no functional <location> value was confirmed. Verify the error page is designated and functional.' ($effective -join "`n") 'Inspect JEUS error-page settings' }; return New-JeusResult 'VULNERABLE' 'JEUS custom error-page evidence was not found (only commented-out or empty definitions).' $evidence 'Inspect JEUS error-page settings' }
