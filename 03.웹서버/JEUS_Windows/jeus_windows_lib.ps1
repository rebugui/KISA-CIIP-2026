...
function Get-JeusBroadAclEvidence {
    ...
    try { $acl = Get-Acl -LiteralPath $Path -ErrorAction Stop } catch { return [pscustomobject]@{ Path = $Path; Role = $Role; Access = 'Unknown'; Principal = 'N/A'; Rights = "ACL read failed: $($_.Exception.Message)"; AclUnknown = $true } }
...
function Invoke-JeusAclCheck {
    ...
    $broad = @($acl | Where-Object { -not $_.AclUnknown })
    $unknown = @($acl | Where-Object { $_.AclUnknown })
    if ($broad.Count -gt 0) {
        return New-JeusResult 'VULNERABLE' "Broad general-user access (read or write) ACL evidence was found on JEUS $Role paths." (($broad | ForEach-Object { "$($_.Path) => $($_.Principal) $($_.Access) ($($_.Rights))" }) -join "`n") "Get-Acl JEUS $Role paths"
    }
    if ($unknown.Count -gt 0) {
        return New-JeusResult 'MANUAL' "JEUS $Role paths could not be read for ACL assessment (Get-Acl failed); broad-access compliance cannot be claimed. Verify the DACL manually." (($unknown | ForEach-Object { "$($_.Path) => $($_.Rights)" }) -join "`n") "Get-Acl JEUS $Role paths"
    }
...
function Invoke-JeusWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'WEB-04' { $lines = @(Get-JeusConfigLines $state '(?i)<allow-indexing>\s*true|<allow-indexing>\s*false|listings'); $stripped = Get-JeusStrippedConfigText $state; if ($stripped -match '(?i)<allow-indexing>\s*true|listings\s*=\s*true') { return New-JeusResult 'VULNERABLE' 'JEUS directory listing appears enabled.' ($lines -join "`n") 'Inspect jeus-web-dd.xml/web.xml directory listing settings' }; if ($stripped -match '(?i)<allow-indexing>\s*false|listings\s*=\s*false') { return New-JeusResult 'GOOD' 'JEUS directory listing disable evidence was found.' ($lines -join "`n") 'Inspect jeus-web-dd.xml/web.xml directory listing settings' }; return New-JeusResult 'MANUAL' 'JEUS directory listing evidence was not conclusive.' $evidence 'Inspect jeus-web-dd.xml/web.xml directory listing settings' }
        ...
        'WEB-10' { $lines = @(Get-JeusConfigLines $state '(?i)ReverseProxy|proxy|PathPrefix|ServerAddress'); if ($lines.Count -gt 0) { return New-JeusResult 'MANUAL' 'JEUS proxy/reverse-proxy evidence was found; confirm only required proxy mappings remain.' ($lines -join "`n") 'Inspect JEUS proxy mappings' }; if ($state.ConfigFiles.Count -eq 0) { return New-JeusResult 'MANUAL' 'JEUS is installed but no config files could be inspected (JEUS_HOME/config unresolved); zero files were examined, so absence of proxy/reverse-proxy mappings cannot be claimed. Review proxy mappings manually.' $evidence 'Inspect JEUS proxy mappings' }; return New-JeusResult 'GOOD' 'No JEUS proxy/reverse-proxy evidence was found in inspected configs.' $evidence 'Inspect JEUS proxy mappings' }
        ...
        'WEB-12' { $lines = @(Get-JeusConfigLines $state '(?i)<aliasing>|<alias>|<alias-name>|<real-path>'); $stripped = Get-JeusStrippedConfigText $state; if ($stripped -match '(?i)<aliasing>|<alias>|<alias-name>|<real-path>') { return New-JeusResult 'VULNERABLE' 'JEUS active alias/link mapping evidence (outside comment spans) was found.' ($lines -join "`n") 'Inspect JEUS aliasing settings' }; if ($state.ConfigFiles.Count -eq 0) { return New-JeusResult 'MANUAL' 'JEUS is installed but no config files could be inspected (JEUS_HOME/config unresolved); zero files were examined, so absence of alias/link mappings cannot be claimed. Review aliasing settings manually.' $evidence 'Inspect JEUS aliasing settings' }; return New-JeusResult 'GOOD' 'No active JEUS alias/link mapping evidence was found in inspected configs (commented-out definitions are treated as disabled).' $evidence 'Inspect JEUS aliasing settings' }
        'WEB-13' { $lines = @(Get-JeusConfigLines $state '(?i)DataSource|db|jdbc|password'); if ($lines.Count -gt 0) { return New-JeusResult 'MANUAL' 'JEUS DB/resource configuration evidence was found; confirm unnecessary DB connection resources and exposed secrets are removed, and that the config file ACL restricts general-user access.' ($lines -join "`n") 'Inspect JEUS domain.xml/resource configs' }; $aclEvidence = @(foreach ($cfg in @($state.ConfigFiles)) { Get-JeusBroadAclEvidence -Path $cfg -Role 'db-config' }); $aclBroad = @($aclEvidence | Where-Object { -not $_.AclUnknown }); $aclUnknown = @($aclEvidence | Where-Object { $_.AclUnknown }); if ($aclBroad.Count -gt 0) { return New-JeusResult 'VULNERABLE' 'No DB keyword matched, but JEUS configuration files grant broad general-user ACL access; DB connection files must be access-restricted.' (($aclBroad | ForEach-Object { "$($_.Path) => $($_.Principal) $($_.Access) ($($_.Rights))" }) -join "`n") 'Get-Acl JEUS config files for DB-file ACL' }; if ($aclUnknown.Count -gt 0) { return New-JeusResult 'MANUAL' 'No DB keyword matched, and JEUS configuration files could not be read for ACL assessment (Get-Acl failed); broad-access compliance cannot be claimed. Verify the DACL and DB connection files manually.' (($aclUnknown | ForEach-Object { "$($_.Path) => $($_.Rights)" }) -join "`n") 'Get-Acl JEUS config files for DB-file ACL' }; return New-JeusResult 'MANUAL' 'No DB/resource keyword matched inspected configs; DB usage may rely on non-standard naming. Confirm no unnecessary DB connection file exists and that any DB config file ACL restricts general-user access.' $evidence 'Inspect JEUS domain.xml/resource configs' }
        ...
        'WEB-16' { $lines = @(Get-JeusConfigLines $state '(?i)serverInfo=false|server-header|ServerTokens'); $stripped = Get-JeusStrippedConfigText $state; if ($stripped -match '(?i)serverInfo=false') { return New-JeusResult 'GOOD' 'JEUS server header suppression evidence was found.' ($lines -join "`n") 'Inspect JEUS serverInfo/header settings' }; return New-JeusResult 'MANUAL' 'JEUS server header suppression evidence was not conclusive.' $evidence 'Inspect JEUS serverInfo/header settings' }
