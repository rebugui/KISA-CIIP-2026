...
function Invoke-WebtoBAclCheck {
    ...
    if ($acl) { return New-WebtoBResult 'VULNERABLE' "Broad general-user access (read or write) or unknown ACL evidence was found on WebtoB $Role paths." (($acl | ForEach-Object { "$($_.Path) => $($_.Principal) $($_.Access) ($($_.Rights))" }) -join "`n") "Get-Acl WebtoB $Role paths" }
...
function Invoke-WebtoBWindowsCheck {
    ...
    switch ($ItemId) {
        ...
        'WEB-16' { $lines = @(Get-WebtoBActiveConfigLines $state '(?i)ServerTokens|ServerSignature'); if ($lines -match '(?i)ServerTokens\s+Prod|ServerTokens\s+ProductOnly|ServerSignature\s+off') { return New-WebtoBResult 'GOOD' 'WebtoB server header minimization evidence was found.' ($lines -join "`n") 'Inspect http.m ServerTokens/ServerSignature' }; return New-WebtoBResult 'MANUAL' 'WebtoB server header minimization evidence was not conclusive.' (($lines + $evidence) -join "`n") 'Inspect http.m ServerTokens/ServerSignature' }
        'WEB-17' { $lines = @(Get-WebtoBConfigLines $state '(?i)^\s*\*ALIAS|RealPath|ALIAS'); if ($lines.Count -gt 0) { return New-WebtoBResult 'VULNERABLE' 'WebtoB virtual/alias directory evidence was found.' ($lines -join "`n") 'Inspect http.m ALIAS settings' }; if (@($state.ConfigFiles).Count -eq 0) { return New-WebtoBResult 'MANUAL' 'WebtoB appears installed but no http.m config file could be read; virtual/alias directory presence could not be determined. Manually inspect http.m ALIAS settings.' $evidence 'Inspect http.m ALIAS settings' }; return New-WebtoBResult 'GOOD' 'No WebtoB virtual/alias directory evidence was found.' $evidence 'Inspect http.m ALIAS settings' }
        'WEB-18' { $lines = @(Get-WebtoBConfigLines $state '(?i)Method\s*=.*(PROPFIND|PUT|DELETE|MKCOL|COPY|MOVE)|WebDAV|dav'); if ($lines.Count -gt 0) { return New-WebtoBResult 'VULNERABLE' 'WebtoB WebDAV-like methods/settings evidence was found.' ($lines -join "`n") 'Inspect http.m WebDAV methods' }; if (@($state.ConfigFiles).Count -eq 0) { return New-WebtoBResult 'MANUAL' 'WebtoB appears installed but no http.m config file could be read; WebDAV activation could not be determined. Manually inspect http.m WebDAV methods.' $evidence 'Inspect http.m WebDAV methods' }; return New-WebtoBResult 'GOOD' 'No WebtoB WebDAV-like method evidence was found.' $evidence 'Inspect http.m WebDAV methods' }
        'WEB-19' { $lines = @(Get-WebtoBConfigLines $state '(?i)SVRTYPE\s*=\s*SSI|SvrType\s*=\s*SSI|SSI'); if ($lines.Count -gt 0) { return New-WebtoBResult 'VULNERABLE' 'WebtoB SSI server mapping evidence was found.' ($lines -join "`n") 'Inspect http.m SSI settings' }; if (@($state.ConfigFiles).Count -eq 0) { return New-WebtoBResult 'MANUAL' 'WebtoB appears installed but no http.m config file could be read; SSI activation could not be determined. Manually inspect http.m SSI settings.' $evidence 'Inspect http.m SSI settings' }; return New-WebtoBResult 'GOOD' 'No WebtoB SSI mapping evidence was found.' $evidence 'Inspect http.m SSI settings' }
        'WEB-20' { $lines = @(Get-WebtoBActiveConfigLines $state '(?i)SSLFLAG\s*=\s*[NY]|SSLNAME|CertificateFile|Protocols'); if ($lines -match '(?i)SSLFLAG\s*=\s*Y') { return New-WebtoBResult 'GOOD' 'WebtoB SSL/TLS is enabled (SSLFLAG=Y).' ($lines -join "`n") 'Inspect http.m SSL settings' }; if ($lines -match '(?i)SSLFLAG\s*=\s*N') { return New-WebtoBResult 'VULNERABLE' 'WebtoB SSL/TLS is disabled (SSLFLAG=N) despite SSL configuration being present.' ($lines -join "`n") 'Inspect http.m SSL settings' }; if ($lines.Count -gt 0) { return New-WebtoBResult 'MANUAL' 'WebtoB SSL configuration directives were found but the SSLFLAG activation state could not be determined.' (($lines + $evidence) -join "`n") 'Inspect http.m SSL settings' }; return New-WebtoBResult 'VULNERABLE' 'WebtoB SSL/TLS activation evidence was not found.' $evidence 'Inspect http.m SSL settings' }
        'WEB-21' { $lines = @(Get-WebtoBActiveConfigLines $state '(?i)URLRewrite\s*=\s*[NY]|URLRewriteConfig|RewriteRule|https://'); if (($lines -match '(?i)URLRewrite\s*=\s*Y') -and ($lines -match '(?i)RewriteRule.*https://|https://')) { return New-WebtoBResult 'GOOD' 'WebtoB HTTP-to-HTTPS redirection is enabled (URLRewrite=Y with an explicit https:// redirect rule).' ($lines -join "`n") 'Inspect http.m rewrite settings' }; if ($lines -match '(?i)URLRewrite\s*=\s*N') { return New-WebtoBResult 'VULNERABLE' 'WebtoB URL rewrite is disabled (URLRewrite=N); HTTPS redirection is not active.' ($lines -join "`n") 'Inspect http.m rewrite settings' }; if ($lines -match '(?i)URLRewrite\s*=\s*Y') { return New-WebtoBResult 'MANUAL' 'WebtoB URLRewrite=Y is set but an explicit HTTPS redirect rule could not be confirmed; verify the rewrite config.' (($lines + $evidence) -join "`n") 'Inspect http.m rewrite settings' }; return New-WebtoBResult 'VULNERABLE' 'WebtoB HTTP-to-HTTPS redirection activation evidence was not found.' $evidence 'Inspect http.m rewrite settings' }
        'WEB-22' { $lines = @(Get-WebtoBActiveConfigLines $state '(?i)ERRORDOCUMENT|\surl\s*='); if ($lines -match '(?i)url\s*=\s*"?[^",\s]+') { return New-WebtoBResult 'GOOD' 'WebtoB custom error page is designated (ERRORDOCUMENT with a url= assignment).' ($lines -join "`n") 'Inspect http.m ERRORDOCUMENT' }; if ($lines -match '(?i)ERRORDOCUMENT') { return New-WebtoBResult 'VULNERABLE' 'WebtoB ERRORDOCUMENT section was found but no custom error page (url=) was designated.' (($lines + $evidence) -join "`n") 'Inspect http.m ERRORDOCUMENT' }; return New-WebtoBResult 'VULNERABLE' 'WebtoB custom error page designation evidence was not found.' $evidence 'Inspect http.m ERRORDOCUMENT' }
