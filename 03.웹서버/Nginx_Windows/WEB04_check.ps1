...
try {
    ...
    elseif ($configFiles.Count -eq 0) {
        $finalResult = "MANUAL"; $status = "수동진단"; $summary = "Nginx process exists, but configuration files were not found for autoindex inspection."; $commandOutput = ($processes | ForEach-Object { "$($_.ProcessId) $($_.CommandLine)" }) -join "`n"; $commandExecuted = "Get-CimInstance Win32_Process; known nginx.conf paths"
    }
    else {
        $config = Get-NginxWindowsConfigText -ConfigFiles $configFiles
        $lines = Get-NginxActiveLines -ConfigText $config.Text
        $enabled = @($lines | Where-Object { $_ -match '^(?i)autoindex\s+on\s*;' })
        $disabled = @($lines | Where-Object { $_ -match '^(?i)autoindex\s+off\s*;' })
        $commandExecuted = "Parse active autoindex directives from Nginx Windows configuration"
        $commandOutput = if ($enabled -or $disabled) { (@($enabled + $disabled) -join "`n") } else { "No active autoindex directives found." }
        if ($enabled.Count -gt 0) { $finalResult = "VULNERABLE"; $status = "취약"; $summary = "Nginx autoindex is enabled." }
        else { $finalResult = "GOOD"; $status = "양호"; $summary = "No active autoindex on directive was found." }
    }
}
catch {
    $finalResult = "MANUAL"
    ...
    $commandExecuted = "Nginx Windows autoindex configuration discovery"
}
