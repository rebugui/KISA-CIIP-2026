...
diagnose() {
    ...
    local error_docs=$(grep -rE "^\s*ErrorDocument\s+(400|401|403|404|500|503)" "${apache_conf}" /etc/httpd/conf.d/ /etc/httpd/conf.modules.d/ /etc/apache2/sites-enabled/ /etc/apache2/conf-enabled/ 2>/dev/null | grep -v "^\s*#" | head -10 || true)
    local error_doc_count=$(echo "${error_docs}" | grep -c "ErrorDocument" || true)

    command_executed="grep -rE '^\\s*ErrorDocument\\s+(400|401|403|404|500|503)' ${apache_conf} /etc/httpd/conf.d/ /etc/httpd/conf.modules.d/ /etc/apache2/sites-enabled/ /etc/apache2/conf-enabled/ 2>/dev/null | grep -v '^\\s*#' | head -10"
