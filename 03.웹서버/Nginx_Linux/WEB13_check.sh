...
# @Description : DB 연결 파일 접근 권한 및 스크립트 매핑 점검. 점검 대상: Tomcat, IIS, JEUS (Nginx 비대상)
...
diagnose() {
    ...
    # WEB-13 점검 대상은 Tomcat, IIS, JEUS 이며 Nginx는 비대상(out-of-target)이다.
    # (docs/guideline_metadata.json WEB-13 target: "Tomcat, IIS, JEUS")
    # 따라서 Nginx_Linux에서는 N/A로 고정한다. (DB 연결 파일/스크립트 매핑 점검은 Nginx에 정의되지 않음)
    local diagnosis_result="N/A"
    local status="N/A"
    local inspection_summary="WEB-13은 Tomcat, IIS, JEUS를 점검 대상으로 하며 Nginx는 비대상입니다(docs/guideline_metadata.json WEB-13 target). DB 연결 파일 노출/스크립트 매핑 점검은 Nginx에 적용되지 않습니다."
    local command_result="WEB-13 target: Tomcat, IIS, JEUS. Nginx is out of scope."
    local command_executed="docs/guideline_metadata.json WEB-13 target platform review"
