...
diagnose() {
    ...
    # 핵심 시스템 디렉토리 + 비표준 SUID 설치 위험 경로(/opt, /var, /tmp, /home) 포함
    local search_dirs=(
        "/usr/bin"
        "/usr/sbin"
        "/bin"
    ...
    while IFS= read -r file; do
        if [ -n "$file" ]; then
            ...
            # (-w 검사는 root 실행 시 항상 참이므로 권한 비트 기반으로 group/other 쓰기 판정)
            if [ "$allowed" = false ]; then
                local writable_bits=false
                case "$perms" in
                    *[2367]) writable_bits=true ;;
                esac
                case "$perms" in
                    ?[0-7][2367][0-7]) writable_bits=true ;;
                esac
                if [[ "$file" =~ \.(sh|bash|pl|py|rb)$ ]] || [ "$writable_bits" = true ]; then
                    ((vulnerable_count++)) || true
                    vulnerable_files="${vulnerable_files}${file} (SUID, 권한: ${perms}, 소유자: ${owner}), "
                else
                    # 허용 목록 외 SUID: 필요성 수동 확인 대상
                    ((manual_count++)) || true
