...
diagnose() {
    ...
    # 2) Postfix: /usr/sbin/postsuper 의 일반 사용자 실행 권한 확인 (others 실행 비트)
    #    (KISA 판단 기준: postsuper 의 o-x 가 설정되면 양호, 조치: chmod o-x /usr/sbin/postsuper)
    local ps_bin=""
    for cand in /usr/sbin/postsuper /usr/lib/postfix/sbin/postsuper; do
        [ -f "$cand" ] && ps_bin="$cand" && break
    done
    if [ -n "$ps_bin" ] || command -v postconf &>/dev/null; then
        mail_service_present=true
        if [ -n "$ps_bin" ]; then
            # stat -c 미지원 HP-UX: perl 사용
            local ps_perms
            ps_perms=$(perl -e 'printf "%04o\n", (stat(shift))[2] & 07777' "$ps_bin" 2>/dev/null || echo "000")
            ...
            needs_manual=true
            restriction_info="${restriction_info}[Postfix] postconf 는 가능하나 postsuper 경로 미발견 - 실행 제한 확인 불가(수동 점검 필요)\\n"
        fi
    fi
