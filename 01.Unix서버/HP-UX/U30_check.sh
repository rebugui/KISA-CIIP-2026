...
diagnose() {
    ...
    local secdef_grep=$(grep "^[[:space:]]*UMASK=" /etc/default/security 2>/dev/null || echo "No UMASK setting in /etc/default/security")
    command_result="[Command: grep -h 'umask' /etc/profile /etc/bash.bashrc /root/.bashrc /root/.profile]${newline}${umask_grep}${newline}[Command: grep '^UMASK=' /etc/default/security]${newline}${secdef_grep}"
    ...
    # HP-UX 기본 정책: /etc/default/security 의 UMASK=NNN 확인 (대문자, = 구분)
    if [ -f /etc/default/security ] && [ -r /etc/default/security ]; then
        ((config_files_checked++)) || true
        local secdef_value
        secdef_value=$(grep "^[[:space:]]*UMASK=" /etc/default/security 2>/dev/null | tail -1 | awk -F= '{print $2}' | tr -d '[:space:]') || true
        if [ -n "$secdef_value" ]; then
            record_umask_value "/etc/default/security" "$secdef_value"
        fi
    fi

    # 최종 판정 (가장 허용적인 값 기준)
    command_executed="grep '^umask' /etc/profile /etc/bash.bashrc /root/.bashrc /root/.profile; grep '^UMASK=' /etc/default/security"
