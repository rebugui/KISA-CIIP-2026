...
diagnose() {
    ...
    # HP-UX 가이드 점검 대상: /etc/snmpd.conf, /etc/SnmpAgent.d/snmpd.conf
    # (get-community-name / set-community-name), net-snmp 설치 시 /etc/snmp/snmpd.conf
    local conf_candidates=("/etc/snmpd.conf" "/etc/SnmpAgent.d/snmpd.conf" "/etc/snmp/snmpd.conf")
    ...
    if [ "$snmpd_installed" = false ]; then
        ...
        local snmp_not_installed=$(ls /etc/snmpd.conf /etc/SnmpAgent.d/snmpd.conf /etc/snmp/snmpd.conf 2>/dev/null || echo "SNMP not installed")
        command_result="${snmp_not_installed}"
        command_executed="ls /etc/snmpd.conf /etc/SnmpAgent.d/snmpd.conf /etc/snmp/snmpd.conf 2>/dev/null"
    elif [ ${#existing_confs[@]} -eq 0 ]; then
        diagnosis_result="GOOD"
        status="양호"
        inspection_summary="SNMP 설정 파일이 존재하지 않음"
        local no_snmp_conf=$(ls /etc/snmpd.conf /etc/SnmpAgent.d/*.conf /etc/snmp/*.conf 2>/dev/null || echo "No SNMP config files found")
        command_result="${no_snmp_conf}"
        command_executed="ls /etc/snmpd.conf /etc/SnmpAgent.d/snmpd.conf /etc/snmp/snmpd.conf 2>/dev/null"
    else
        # 2) 설정 파일별 Community String 값 추출 후 복잡성 판정
        #    - HP-UX 네이티브 : get-community-name / set-community-name (콜론 뒤 첫 토큰)
            ...
            file_values=$(awk '
                {
                    l = tolower($0)
                    if (l ~ /^[[:space:]]*#/) next
                    if (l ~ /^[[:space:]]*(get|set)-community-name/) {
                        line = $0
                        if (line ~ /:/) {
                            sub(/^[^:]*:[[:space:]]*/, "", line)
        ...
        if [ "$weak_community" = true ]; then
            ...
            # Community 값 미추출(v3 전용 포함): HP-UX 기본 에이전트는 get-community-name 미설정 시
            # 기본값 'public'을 적용할 수 있고, v3 사용자 비밀번호 복잡성은 정적으로 검증 불가 → 수동진단
            diagnosis_result="MANUAL"
            status="수동진단"
            inspection_summary="SNMP Community String이 설정 파일에서 확인되지 않음 - HP-UX 기본 에이전트는 미설정 시 기본 community(public) 적용 가능하며 v3 비밀번호 복잡성은 정적 확인 불가, 수동 점검 필요"
            command_result="${raw_output}"
            command_executed="grep -iE 'community|com2sec' ${existing_confs[*]}"
        fi
    fi
