...
diagnose() {
    ...
    local logindefs_grep=$(grep "^[[:space:]]*UMASK[[:space:]]" /etc/login.defs 2>/dev/null || echo "No UMASK setting in /etc/login.defs")
    command_result="[Command: grep -h 'umask' /etc/profile /etc/bash.bashrc /root/.bashrc /root/.profile]${newline}${umask_grep}${newline}[Command: grep '^UMASK' /etc/login.defs]${newline}${logindefs_grep}"
    ...
    # 최종 판정 (가장 허용적인 값 기준)
    command_executed="grep '^umask' /etc/profile /etc/bash.bashrc /root/.bashrc /root/.profile; grep '^UMASK' /etc/login.defs"
