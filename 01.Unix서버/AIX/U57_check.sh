...
diagnose() {
    ...
    if [ -n "$proftpd_conf" ] && [ "$proftpd_root_login" = "on" ]; then
        ...
        # ftpusers 파일에 root가 활성 라인(주석/공백 제외)으로 등록되어 있으면 양호
        local active_lines=$(echo "$file_content" | grep -v "^#" | grep -v "^$" || true)
        if echo "$active_lines" | grep -qx "root"; then
            diagnosis_result="GOOD"
            status="양호"
            inspection_summary="ftpusers 파일에 root 계정이 등록되어 root FTP 접속이 차단됩니다(${found_file})."
            if [ ${#missing_accounts[@]} -ne 0 ]; then
                local missing_list=""
                for acc in "${missing_accounts[@]}"; do
                    missing_list="${missing_list}${acc}, "
                done || true
                inspection_summary="${inspection_summary} (참고: 미등록 시스템 계정: ${missing_list%, })"
            fi
        else
            diagnosis_result="VULNERABLE"
            status="취약"
