...
diagnose() {
    ...
    # 1-2) FTP 서비스 동작 확인 (AIX: lssrc)
    if lssrc -s vsftpd 2>/dev/null | grep -q "active" || lssrc -s proftpd 2>/dev/null | grep -q "active" || lssrc -s pure-ftpd 2>/dev/null | grep -q "active"; then
        ftp_running=true
        evidence="${evidence}[FTP 서비스] 실행 중(lssrc)${newline}"
    fi
    ...
    # ==================== 2) NFS 익명/전체 공유 (AIX: /etc/exports, anon!=-1 취약) ====================
    ...
    # ==================== 3) Samba 익명(guest) 접근 (AIX: /usr/lib/smb.conf) ====================
    local smb_conf=""
    for smb_conf in /usr/lib/smb.conf /etc/samba/smb.conf; do
        if [ -f "$smb_conf" ]; then
            if [ -r "$smb_conf" ]; then
                local smb_guest=""
    ...
    command_executed="grep -E '^(ftp|anonymous):' /etc/passwd; grep -i anonymous_enable /etc/vsftpd.conf; grep -i '<Anonymous' /etc/proftpd.conf /etc/proftpd/proftpd.conf; grep anon /etc/exports; grep -iE 'guest ok|map to guest' /usr/lib/smb.conf /etc/samba/smb.conf"
