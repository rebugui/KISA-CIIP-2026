...
diagnose() {
    ...
    if command -v svcs >/dev/null 2>&1; then
        ...
        if [ -n "$ssh_status_output" ]; then
            # 상태 라인에서 "online" 확인
            local ssh_state=$(echo "$ssh_status_output" | grep -E "^[[:space:]]*state[[:space:]]" | awk '{print $2}' || echo "")
    ...
    if command -v svcs >/dev/null 2>&1; then
        ...
        if [ -n "$telnet_status_output" ]; then
            local telnet_state=$(echo "$telnet_status_output" | grep -E "^[[:space:]]*state[[:space:]]" | awk '{print $2}' || echo "")
    ...
    if [ "$telnet_checked" = false ] && command -v inetadm >/dev/null 2>&1; then
        ...
        if [ -n "$inetadm_output" ]; then
            # enabled 상태 확인
            # Solaris inetadm 출력은 들여쓰기 + "enabled=TRUE" 형식(대문자) → 공백/대소문자 모두 허용
            local telnet_enabled=$(echo "$inetadm_output" | grep -E "enabled[=[:space:]]" | grep -v "^[[:space:]]*#" | head -1 | sed -E 's/.*enabled[[:space:]]*=?[[:space:]]*//' | awk '{print $1}' | tr 'A-Z' 'a-z' || echo "")
    ...
    if [ "$telnet_active" = true ]; then
        # Solaris 기준: /etc/default/login의 CONSOLE 설정으로 root 직접 로그인 차단
        # CONSOLE=/dev/console 이 주석 해제되어 있으면 root는 콘솔에서만 로그인 가능 (양호)
        local default_login_file="/etc/default/login"
        if [ -f "$default_login_file" ]; then
            console_output=$(grep -E "^[[:space:]]*CONSOLE[[:space:]]*=" "$default_login_file" 2>/dev/null | head -1 || true)
            if [ -n "$console_output" ]; then
                telnet_secure=true
                telnet_details="[Telnet] /etc/default/login CONSOLE 설정 존재 (root 원격 로그인 차단, 양호)"
            else
                telnet_secure=false
                telnet_details="[Telnet] /etc/default/login CONSOLE 설정 없음/주석 처리 (취약)"
                console_output="설정 없음 (주석 처리 또는 미설정)"
            fi
        else
            telnet_secure=false
            telnet_details="[Telnet] /etc/default/login 파일 없음 (취약)"
            console_output="파일 없음"
        fi
    else
    ...
    # Telnet 부분 추가
    if [ "$telnet_active" = true ]; then
        command_result="${command_result}${telnet_service_output}[/etc/default/login]${newline}${console_output}"
        command_executed="${command_executed}; svcs -l svc:/network/telnet:default; inetadm -l telnet; grep -E '^[[:space:]]*CONSOLE[[:space:]]*=' /etc/default/login"
    else
    ...
    # -------------------------------------------------------------------------
    # 5. 최종 판정 (앞 단계에서 MANUAL로 확정된 경우 유지)
    # -------------------------------------------------------------------------
    if [ "$diagnosis_result" = "MANUAL" ]; then
        : # SSH 설정 파일 부재 등 수동진단 판정 유지
