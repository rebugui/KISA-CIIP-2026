...
diagnose() {
    ...
    # BIND 설정 파일 경로 확인 (Solaris 실제 경로 + Debian 계열 추가 경로)
    ...
    service_probe=$( { svcs -H -o state svc:/network/dns/server 2>/dev/null; ps -ef 2>/dev/null | grep -w "named" | grep -v "grep"; } | tr -d '\r' || true)
    if svcs -H -o state svc:/network/dns/server 2>/dev/null | grep -q "online"; then
    ...
    if [ "$dns_configured" = false ]; then
        ...
        command_result="[Command: svcs -H -o state svc:/network/dns/server; ps -ef | grep -w named]${newline}${service_probe:-DNS service not found}${newline}[Config] /etc/named.conf, /etc/named.boot, /var/named/named.conf, /etc/bind/named.conf* 없음"
        command_executed="svcs -H -o state svc:/network/dns/server; ps -ef | grep -w named; ls /etc/named.conf /etc/named.boot /var/named/named.conf /etc/bind/named.conf*"
        ...
        command_executed="svcs -H -o state svc:/network/dns/server; ps -ef | grep -w named; sed -n '/allow-update/,/;/p' <named.conf>"
    fi
